...
# Config for OPC-UA and InfluxDB
opcua_url = "opc.tcp://192.168.1.100:4840"  # OPC-UA server URL (configured from )
bucket = "plc_data"                         # InfluxDB bucket name (similar to a database)
...
try:
    while True:
        try:
            # Read live values from all OPC-UA nodes
            tag_data = {
                k: node.get_value() for k, node in nodes.items()
            }

            # Create an InfluxDB Point with the tag and field data (Point is like a row of data)
            point = (
                Point("plc_telemetry")
                .tag("device", "PLC1")
    ...
    # OPC-UA client disconnects on exit
