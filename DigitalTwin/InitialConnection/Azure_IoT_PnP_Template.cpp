...
/* --- Internal Functions --- */
static float get_temperature_from_sensor() {
    sensors.requestTemperatures();  // Request a temperature reading from the sensor
    float temperature = sensors.getTempCByIndex(0);  // Get temperature in Celsius for the first sensor (index 0)
    return temperature;
}
    ...
    size_t* payload_buffer_length)
{
  ...
  // Acquiring the data from loop.
  float temperature = get_temperature_from_sensor();
