...
/* --- Defines --- */
// extern const int motionSensorPin
...
#define TELEMETRY_PROP_NAME_MOTION "motionDetection"
    ...
    size_t* payload_buffer_length)
{
  ...
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding motion sensor property name to telemetry payload.");
  // rc = az_json_writer_append_double(&jw, temperature, DOUBLE_DECIMAL_PLACE_DIGITS);
  rc = az_json_writer_append_int32(&jw, motionState);
  EXIT_IF_AZ_FAILED(
      rc, RESULT_ERROR, "Failed adding motion sensor property value to telemetry payload. ");
